...
updates: -MAC 11/5/17: refactoring
*/
double GRNN::classify(Data_Point trng_set[TRAINING_SET], Data_Point point, double weights[]) {
	double gsan = 0, wgtd_gsan = 0;
	int i;
	for(i = 0; i < TRAINING_SET; i++) {
		if(point.distance(trng_set[i], weights) != 0) {
			gsan += exp((-1 * pow(point.distance(trng_set[i], weights), 2)) / (2 * pow(sigma, 2)));
			wgtd_gsan += exp((-1 * pow(point.distance(trng_set[i], weights), 2)) /
